...
test("keeps every header tier frosted and removes the hero guide frame", async () => {
  ...
  assert.match(
    css,
    /@media \(max-width: 1100px\)\s*\{\s*\.site-header\s*\{[^}]*background:\s*rgba\(3,\s*5,\s*7,\s*\.78\);[^}]*backdrop-filter:\s*blur\(6px\)\s*saturate\(112%\)/s,
  );
  assert.match(
    css,
    /@media \(max-width: 760px\)\s*\{\s*\.site-header\s*\{[^}]*background:\s*rgba\(3,\s*5,\s*7,\s*\.64\);[^}]*backdrop-filter:\s*blur\(10px\)\s*saturate\(120%\)/s,
  );
